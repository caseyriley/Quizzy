      ...
      fetch(`${API_URL}/questions`, {
        ...
        .then(() => {
          ...
          for (let i = 0; i < numQ; i++) {
            let answersArray = [
              quizz[i]["qna"].incorrect[0],
              quizz[i]["qna"].incorrect[1],
              quizz[i]["qna"].incorrect[2],
              quizz[i]["qna"].correct,
            ];
      ...
      <div id={"main"} className={"fade-in"}>
        ...
        {errorInstState === false && (
          <div className={"q-instructions"}>
            <p>
              Answer 10 multiple choice questions and click the submit button to
              see your score.
            </p>
            <p>
              The correct answers will be displayed only after submitting your
              answers.
            </p>
            <p>
              The questions provided are random so each time you play the
        ...
        {errorInstState === true && (
          <div className={"q-instructions q-instructions-err"}>
            <p>
              Answer any unanswered questions before submitting your answers
            </p>
          </div>
        )}
          ...
          [...Array(10)].map((el, i) => {
            return (
              <Question
                key={i}
                ky={i}
                question={quizzState[i]["qna"].question}
                answers={questionsState[`a${i}`]}
                correct={quizzState[i] && quizzState[i]["qna"].correct}
